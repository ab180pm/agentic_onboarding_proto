...
**모든 온보딩 과정은 Chat UI 내에서 완결되어야 합니다.**

| 금지 사항 | 대안 |
|-----------|------|
| ❌ "대시보드에서 확인해 주세요" | ✅ Chat UI에서 직접 정보 표시 |
| ❌ "Settings → Tokens에서 토큰 확인" | ✅ 앱 등록 시 자동으로 토큰 생성/표시 |
| ❌ "Dashboard를 열어서 설정하세요" | ✅ Chat UI에서 설정 완료 후 결과 표시 |
| ❌ "Apple Developer Dashboard 확인" | ✅ 필요한 정보 입력 UI 제공 |
| ❌ 외부 사이트로 이동 후 정보 확인 요청 | ✅ Chat UI에서 입력받거나 자동 추출 |

**구현 원칙:**
1. **토큰/인증 정보**: 앱 등록 완료 시 Chat UI에 직접 표시 (복사 버튼 포함)
2. **SDK 설정값**: 이전 단계에서 수집한 정보로 자동 채움
3. **딥링크 설정**: Chat UI에서 입력받은 정보를 API로 자동 등록
4. **채널 연동**: OAuth 연동 후 결과를 Chat UI에 표시

**예외 (허용되는 경우):**
- 최종 완료 후 대시보드 리포트 확인 링크 제공
- OAuth 인증 플로우 (팝업 후 자동 복귀)
- 외부 서비스 계정 생성이 필요한 경우 (Meta Business, Google Ads 등)
...
### Phase 1-4: 역할 확인 및 분기 (토큰 확인 후)

앱 등록 완료 후 토큰이 표시되면, 사용자의 역할을 확인하여 적절한 경로로 안내합니다.

#### 1-4-1. SDK 설치 가능 여부 확인

**Agent 발화**
```
🛠️ 다음: SDK 설치

Airbridge SDK를 앱 코드베이스에 통합해야 해요.
이건 주로 개발자가 진행하는 기술 작업이에요.
```

**인터페이스: RoleCapabilityCheck**
| SDK 설치를 직접 하실 수 있나요? |
|---|
| ○ **네, 직접 할 수 있어요** |
| └ 코드베이스 접근 권한이 있고 SDK를 직접 통합할 수 있어요 |
| |
| ○ **아니요, 개발자가 할 거예요** |
| └ 마케터 또는 PM이에요 - 기술 설정은 다른 분이 진행해요 |

---

#### 1-4-2. 마케터 경로 (개발자 아닌 경우)

사용자가 SDK 설치를 직접 할 수 없다고 답한 경우, 대안을 제시합니다.

**Agent 발화**
```
👍 괜찮아요! 개발자가 SDK 통합을 진행하는 동안,
지금 바로 할 수 있는 일들이 있어요.
```

**인터페이스: MarketerNextSteps**
| SDK 설치는 개발자가 진행해야 해요. |
| 하지만 지금 바로 할 수 있는 것들이 있어요! |
|---|
| ○ **개발자 초대하기** |
| └ 개발팀에 설정 가이드 공유 |
| |
| ○ **트래킹 링크 만들기** ⭐ 추천 |
| └ SDK 대기 중에 캠페인 링크 미리 생성 |
| |
| ○ **대시보드 둘러보기** |
| └ 리포트와 설정 미리 살펴보기 |

**분기 처리:**
- **개발자 초대하기** → SDK 가이드 공유 화면 (`sdk-guide-share`)
- **트래킹 링크 만들기** → 트래킹 링크 생성 플로우 (`tracking-link-create`)
- **대시보드 둘러보기** → 대시보드 탐색 안내

---

### Phase 2: SDK Installation

> **Note**: 이 Phase는 사용자가 "네, 직접 할 수 있어요"를 선택한 경우에만 진행됩니다.
...
│   └─ Web → Web SDK 가이드
│
├─ Q: "SDK 설치를 직접 하실 수 있나요?" ⭐ NEW
│   ├─ 네, 직접 할 수 있어요 → Phase 2 (SDK Installation)
│   │   └─ 프레임워크 선택 → SDK 설치 가이드
│   │
│   └─ 아니요, 개발자가 할 거예요 → 마케터 경로
│       ├─ 개발자 초대하기 → SDK 가이드 공유
│       ├─ 트래킹 링크 만들기 → 링크 생성 플로우 ⭐ 추천
│       └─ 대시보드 둘러보기 → 대시보드 탐색 안내
│
├─ Q: "어떤 프레임워크인가요?" (개발자 경로)
│   ├─ Native iOS → iOS SDK
...
| 역할 | SDK 설치 | 다음 단계 |
|------|---------|----------|
| 개발자 | 직접 설치 | 프레임워크 선택 → SDK 가이드 |
| 마케터 | 개발자에게 위임 | 트래킹 링크 생성 또는 개발자 초대 |
| PM | 개발자에게 위임 | 대시보드 탐색 또는 개발자 초대 |

**마케터 경로의 핵심:**
- SDK 설치를 기다리는 동안에도 마케터가 할 수 있는 작업 안내
- 트래킹 링크 생성을 **추천 옵션**으로 강조
- 개발자 초대 시 상세 가이드 자동 생성
